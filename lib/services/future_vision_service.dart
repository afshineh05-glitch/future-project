...
class FutureVisionService {
  ...
  Future<FutureVisionState> load() async {
    ...
    final stage = _stageFor(behavior, today);
  ...
  VisionJourneyStage _stageFor(VisionBehaviorSummary behavior, DateTime today) {
    final dates = behavior.activeBehaviorDates;
    if (dates.isEmpty) return VisionJourneyStage.starting;
    final spanDays = dates.last.difference(dates.first).inDays + 1;
    final daysSinceLatest = _day(today).difference(dates.last).inDays;
    if (dates.length >= 30 &&
        spanDays >= 56 &&
        behavior.behaviorCategoryCount >= 2 &&
        daysSinceLatest <= 7) {
  ...
  String _futureSelfMessage({
    required VisionJourneyStage stage,
    required VisionReflection? todayReflection,
    required bool returnedAfterGap,
  }) {
    if (todayReflection?.response == 'not_today') {
      return 'One day doesn\'t decide who you become. What you do next still matters.';
    }
    if (returnedAfterGap) {
      return 'Missing a few days didn\'t erase me. Coming back is part of becoming me.';
    }
    return switch (stage) {
      VisionJourneyStage.starting =>
        'You don\'t need to see the whole path yet. You just need to keep moving toward me.',
      VisionJourneyStage.building =>
        'Every time you follow through, you make me a little more real.',
      VisionJourneyStage.becoming =>
        'You\'re not just working toward me anymore. You\'re starting to live like me.',
      VisionJourneyStage.livingIt =>
        'You kept showing up even when motivation wasn\'t there. That\'s how I became real.',
    };
  }
