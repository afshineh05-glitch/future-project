...
$valid = $items.Count -eq 412 -and
  ...
  $missingAssets.Count -eq 0 -and $brokenRefs.Count -eq 0 -and
  $pending.Count -eq 0 -and
  ($validatedMetadata.Count + $reviewMetadata.Count) -eq 412
...
switch ($Command) {
  ...
  'pilot' { $items | Where-Object {$_.source_name -in @('Arc Trainer','Barbell Bench Press','Barbell Bent Over Row','Barbell Overhead Press','Barbell Romanian Deadlift','Barbell Squat','Dumbbell Curl','Forward Lunge','Hand Plank','Neutral-Grip Pull-Up')} | Select-Object canonical_id,source_name,movement_pattern,metadata_status }
  'validate' {
    [pscustomobject]@{
      ...
      metadata_validated=$validatedMetadata.Count; metadata_pending=$pending.Count
      metadata_needs_review=$reviewMetadata.Count
  ...
  'report' {
    ...
    $existingReview = if (Test-Path -LiteralPath "$directory/review_queue.json") {
      Get-Content -LiteralPath "$directory/review_queue.json" -Raw | ConvertFrom-Json
    } else { $null }
    [ordered]@{
      duplicate_candidates=@($duplicateGroups|ForEach-Object {$_.Group.source_name})
      unresolved=if($existingReview){@($existingReview.unresolved)}else{@($unresolved|Select-Object canonical_id,source_name,validation_status,review_reason)}
