  ...
  Future<void> _generateTrainingPlan() async {
    ...
    try {
      ...
      if (response.status < 200 || response.status >= 300) {
        throw Exception(
          'Generator returned ${response.status}: ${response.data}',
        );
      }
      ...
      if (!mounted) return;
      setState(() {
        error = 'Could not generate Training Plan: $e';
      });
    } finally {
      if (mounted) {
        setState(() {
          ...
          children: [
            if (loading)
              ...
              _EmptyTrainingPlan(
                ...
                onGenerate: _generateTrainingPlan,
              )
            else ...[
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '$level • $weeks-week intelligent cycle',
                  style: const TextStyle(
                    color: AppTheme.textSecondary,
                    fontSize: 13,
                  ),
                ...
                const SizedBox(height: 7),
                const Text(
                  'Built from your profile, priorities and training strategy.',
                  style: TextStyle(
                    color: AppTheme.textSecondary,
                    fontSize: 11,
                    height: 1.35,
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final fittedWidth = (available - gap * (count - 1)) / count;
        final canFit = fittedWidth >= 72;
        ...
        return SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (_, index) => buildDay(index, width: 92),
          ),
        );
      },
    );
...
class _TrainingDayCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final exercise in exercises) {
      for (final muscle in _stringList(exercise['primary_muscles'])) {
        if (!primaryMuscles
            .map((e) => e.toLowerCase())
            .contains(muscle.toLowerCase())) {
